        ...
        <div className="flyer-gallery-lightbox" onClick={onClose}>
            <div className="lightbox-content" onClick={e => e.stopPropagation()}>
                ...
                <div className="image-container">
                    {hasPrev && (
                        <button className="nav-button prev" onClick={onPrev}>&larr;</button>
                    )}

                    <img src={image.full} alt={image.title} />

                    {hasNext && (
                        <button className="nav-button next" onClick={onNext}>&rarr;</button>
                    )}
                </div>

                <div className="image-info">
                    {image.title && <h3>{image.title}</h3>}
                    {image.event_date && <p><strong>Date:</strong> {formatDate(image.event_date)}</p>}
                    {image.venue && <p>Venue: {image.venue}</p>}
                    {image.performers && (
                        <div className="performers-section">
                            <p><strong>Bands/Performers:</strong>
